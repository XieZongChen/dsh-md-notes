...
- **笔记搜索**：管理器顶部搜索框，跨全部工作区搜标题 + 正文（多关键词 AND、大小写不敏感）；结果按工作区分组、命中行高亮，点命中行在编辑器中定位到该行并选中关键词；笔记行另有「在侧栏查看」动作（dsh ≥ 0.1.5-rc）。
- **右侧栏笔记查看器**（dsh ≥ 0.1.5-rc）：对话文件链接 / 文件树 / 搜索 / `@` chip 点击都能把笔记以 markdown 渲染打开在 dsh 右侧停靠面板；互链可直接跳转新 tab；正文中的本地图片内联显示（编辑仍在管理器）。
- **回答操作栏**（复制按钮旁）→ 选择或新建一篇笔记，把该段对话（用户提问 + 回答）**即时**追加进去——文本直接取自对话本身，无需等待；分段标签跟随界面语言（思考内容不记入，只保留最终回答）。
- **对话引用笔记（`@`）**：输入 `@` 选择笔记（支持跨工作区，候选行带插件 logo），点击已插入的 chip 可在右侧栏预览；发送时插件后端把笔记内容注入模型上下文——模型直接引用，无需提示它读取文件；引用行只含相对路径，不泄露本机目录。
...
- **agent 记忆（实验性，默认关闭）**：打开 `agentTools` 后，模型可自己调用 `note_search` / `note_read` / `note_write` 查与记笔记（写入**只追加不覆盖**）。**默认关闭**，因为「能提升问答效果」这一主张尚未验证；见[已知局限](#已知局限known-limitations)。
...
**版本对齐规则**：插件只对齐 dsh 的**稳定版**——`rc` 与（将来的）`final`。dsh 的
npm `latest` 指向的就是 rc，即用户实际安装的版本；`alpha` 约 1-2 天一发、很快被同线
rc 收口，因此**不逐版适配**：一条 minor 线的 alpha 由该线 rc 的一次校验整体覆盖，
仅当插件要使用某个 alpha 的新能力、或追认破坏性变更时才定点检查。下表因此只会出现
rc/final 版本（早期 alpha 行为历史记录保留）。
...
**未发布版本（`NEXT_VERSION`）**：适配 dsh `0.1.7-rc.1`（Session V4 的 producer-owned
消息来源；`*16` 图标族改名 `*Medium`），并**要求 dsh ≥ `0.1.7-rc.1`**——0.1.7 用
`SettingsForms` + `volatile` Config 字段取代了 `ctx.settings.register()`，为它构建的插件在
更旧的 dsh 上无法加载。因此在它发布前，**dsh `0.1.5-rc.2` 请继续用 0.13.0**。
...
| `agentTools` | `'off'` | 是否让模型自己查/写笔记：`'off'` 关闭（默认）/ `'read'` 只查不写 / `'write'` 可查可写。**默认关闭**——这条能力「能否提升问答效果」尚未验证，详见[已知局限](#已知局限known-limitations)。 |

插件配置**不含环境变量，也不涉及任何密钥**。

## 已知局限（Known limitations）

- **agent 记忆默认关闭，且价值未经验证**：`agentTools` 提供「模型自己查/写笔记」的实验能力，
  但「它能提升问答效果」这个主张**没有实测结论**（评测设计与预先定好的判决线见
  [docs/memory-eval.md](docs/memory-eval.md)，尚未执行）。因此默认 `'off'`：默认安装的行为
  与加该能力之前逐字节一致。想试的人显式打开，**并请自己判断是否值得**。
- **无头 / SDK / ACP 部署不支持**：插件依赖 `webServer` 服务，而它只由 dsh 的 **web-app**
  bundle 提供；`dsh --profile sdk` 之类的无头 profile 里插件不会加载（笔记与记忆都不可用）。
  解锁条件见 [docs/TODO.md](docs/TODO.md) §0.9。
- **图片不会随 Git 同步**：图片存在 `.dsh-notes/assets/`，而 Git 同步只镜像 `.md`
  （`git.ts` 的同步/冲突逻辑是文本语义）。当前图片是本机文件，换设备不可见。
- **只能跑在 Web 形态**：右侧栏查看器需要 dsh ≥ 0.1.5-rc；更旧的构建上该功能静默停用。
- **笔记按工作区隔离**：没有「跨项目的个人笔记库」，笔记必须归属某个工作区。
- **dsh 版本绑定**：本插件要求 dsh ≥ `0.1.7-rc.1`（0.1.7 换了设置 API 并引入 `volatile`
  schema，旧版请用 0.13.0）。dsh 不做向下兼容，升级 dsh 前请看[兼容性](#兼容性compatibility)。
