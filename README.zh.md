...
- **回答操作栏**（复制按钮旁）→ 选择或新建一篇笔记，把该段对话（用户提问 + 回答）追加进去，分段标签跟随界面语言（思考内容不记入，只保留最终回答）。
...
**规划中**（见 [docs/TODO.md](docs/TODO.md)）：Git 冲突渲染与可视化解决、笔记能力增强（搜索 / 标题目录 / 互链反链等）、交互体验优化（未推送提醒、记入异步化等）。

## 兼容性（Compatibility）

- **已验证插件版本**：0.4.0（历史见 [CHANGELOG.zh.md](CHANGELOG.zh.md)）。
- **已验证dsh版本**：deepseek-harness mainline `0.1.0-rc.7`。
