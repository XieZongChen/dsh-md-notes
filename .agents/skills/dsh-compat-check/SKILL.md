...
### 5.0 入表前置校验：当前插件版本必须「已定版」

对照表记录的是「**已发布**的插件版本 ↔ dsh 版本」。若跑检查时 CHANGELOG 顶部仍有
`## NEXT_VERSION` 块（说明代码里存在**尚未发版的改动**——它们将作为下一个版本发布），
当前 `package.json` 的 `version` 是旧版本号，此时**不能入表**，否则会把「新代码」记成
「旧插件版本」的验证（假行）：

```sh
grep -n "^## NEXT_VERSION" CHANGELOG.zh.md
```

- **有 NEXT_VERSION（存在未发布改动）** → 本次检查结果**不入表**：在 `docs/TODO.md`
  最上方记兼容条目并注明「适配已通过，待 <将发布版本> 发版后再入表」；向用户说明应先
  「发版」再跑兼容性检查完成入表。影响判定照常进行（供用户决定是否调整代码），但表格不动。
- **无 NEXT_VERSION（已定版，`package.json` 版本 = CHANGELOG 最新版本）** → 可入表。
...
1. **`docs/compatibility.zh.md`（唯一数据源，中文版）**：
   ...
   - **实测 vs 链式推断**：本次检查直接验证的当前插件版本 ↔ 当前 dsh 是**实测**（备注
     「当前最新」）。若本次影响判定为「无影响」，且区间 dsh 变更对插件契约**零变化**
     （`<VER>` 与上一已入表 dsh 版本之间无契约面改动），则**上一插件版本 ↔ 当前 dsh**
     也可入表/保留为**链式推断行**——备注如「链式推断：`0.10.0`↔`<上一dsh>` 实测 +
     `<上一dsh>`→`<当前dsh>` 契约零变」；多个历史插件版本同理逐级推断（每级都须满足
     契约零变）。无法确证契约零变时，旧插件版本行**不**指向新 dsh。
...
- **先发版、后入表**：对照表是「已发布插件版本」的验证记录。跑兼容性检查前若 CHANGELOG
  有 `NEXT_VERSION`（存在未发布改动），本次结果不入表（§5.0），先「发版」再检查入表——
  否则会出现「新代码记成旧版本号」的假行（如 0.10.0↔alpha.5 实际是 0.10.1 才验证的）。
- **链式推断标注**：无影响且区间契约零变时，历史插件版本可指向当前 dsh，但备注必须写清
  「链式推断 + 依据的上一实测组合」；不确证契约零变就不要链式推断。
