...
- **对话中引用笔记（`@`）**：输入框输入 `@` 选择笔记——选中的笔记成为 chip，发送时序列化为
  本地化可读行，路径相对当前工作区根（同工作区 `.dsh-notes/xxx.md`，跨工作区
  `../<目录>/.dsh-notes/xxx.md`——`<工作区名>/…` 前缀实测无法解析，cwd 即工作区）；
  **host 端 `agent/pre-step` 把被引用笔记的内容注入模型请求**（不依赖模型自觉 read），模型直接
  拿到内容并在回答中引用；chip 标签前置截断（>6 字符）且单元格经插件
