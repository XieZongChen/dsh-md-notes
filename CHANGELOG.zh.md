...
- **对话中引用笔记（`@`）**：输入框输入 `@` 选择笔记——选中的笔记成为 chip，发送时序列化为
  本地化可读行（同工作区为相对路径，如 `引用笔记「标题」：.dsh-notes/xxx.md`，跨工作区为
  绝对路径），模型用 `read` 工具读取笔记内容并在回答中引用；chip 标签前置截断（>6 字符）且
  单元格经插件 `@font-face` 覆盖放大（4em→6em），短标签外观窄、长标题可读；跨工作区用
  `@工作区名/`（仅 ASCII 工作区名）；被引用笔记已删除/移动时阻断发送并提示
  「无法找到，请删除引用」。
