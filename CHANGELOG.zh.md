...
- **记入笔记携带产物与图片**：把回答记入笔记时，回答中 AI 产出的文件（`write` / `edit` /
  `str_replace_editor` 写出的路径）以笔记目录相对链接带入（`📎 产出文件` 小节，去重）；
  回答中的**图片经插件自有路由原位引用——零复制**（URL 携带持久附件引用，host 按需从
  dsh 附件库读字节，不向任何目录写文件；预览/查看器内联渲染）。局限：图片字节留在本机
  附件库，笔记经 Git 同步到其它机器后文字在、图片不显示（如需跨机器图片，后续可加
  「拷贝进仓库」选项）。见 [docs/usage.zh.md §3](docs/usage.zh.md#3-把对话记入笔记)。
