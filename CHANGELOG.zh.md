...
- **笔记成为 agent 可自己查、自己记的记忆**：新增三个 agent 工具——`note_search`
  按关键词跨工作区搜笔记（不传关键词时列出最近的笔记）、`note_read` 读全文、
  `note_write` 追加持久事实（**只追加、绝不覆盖**；只有新文件才允许 `create`）。每次
  会话开头注入一条短提示告诉模型笔记库存在，因此**不再需要用户先用 `@` 把笔记贴上去**，
  模型可以自己决定去查。`Config.agentTools` 可切档：`write`（默认）/ `read`（agent 只查
  不写）/ `off`（回到纯文档管理器）。设计与动机见 [docs/memory.md](docs/memory.md)。
...
- **升级 dsh ≥ `0.1.5-alpha.1` 后，旧 dsh 上用过 `@` 笔记引用的历史会话无法打开**：dsh
  ...
  新版插件在旧 dsh（V2 日志）上写入的会话也能平滑迁移。设置面板现提供
  **「历史会话修复」**一键改写（见下 Added），可就地恢复受影响的旧会话。

### Added

- **设置面板新增「历史会话修复」**：一键扫描全部本地会话日志，把旧版本插件写入的
  `md-notes` 消息来源改写为官方 `plugin` **最小形态**（按 v0→v1 迁移的精确键集去掉 `path` 等额外成员；可重复执行、二次清理），使被 dsh 0.1.5+ 拒绝打开的历史会话恢复
  可读。原文件逐个在同目录备份（`*.dsh-md-notes-repair.bak`）、原子替换、无需重启；结果
  内联回报（扫描/修复数、失败数、以及 dsh 自身历史来源类型导致的仍不可读提示）。见
  [docs/usage.zh.md §6](docs/usage.zh.md#6-设置面板)。
...
- **@ 候选菜单行首图标换插件 logo**：与 chip 上的 logo 呼应（此前为通用文件图标）。
...
- **预览互链新增 markdown 路径链接解析**：注入约定教模型用 `[标题](路径)` 引用笔记，
  这类链接随回答「记入笔记」后原样留在笔记里，预览却只认 `` `名` `` / `[[名]]`` 两种
  写法——现在 `[任意文字](.dsh-notes/…)`（含 `../<工作区>/…` 跨工作区与深度不严格匹配的
  模型笔误形式）也会改写成可点互链；图片链接与未知目标保持原样。管理器预览与侧栏
  查看器同步生效。
- **点击跨工作区笔记 chip 崩溃（查看器不弹、引用删不掉）**：跨工作区引用的预览地址带
  `..` 段，而侧栏 tab 路由的 picomatch `**` 不匹配 `..`——没有任何类型认领，`claim`
  抛错并穿透进 Lexical 命令处理，把编辑器状态打坏（后续一串 #20 报错、chip 无法删除）。
  现在跨工作区引用经工作区快照解析为**绝对地址**（纯内存导航参数，不进草稿/消息/笔记/
  日志，无隐私面）；快照未就绪或任何打开失败都安全拒绝（点击无副作用），绝不再向
  编辑器抛异常。
- **启动后首页 `@` 菜单无笔记（需刷新才恢复）**：`dsh web` 刚启动时自动打开的页面上，
  草稿会话尚未在 host 侧物化，`@` 的会话级笔记列表把这次「暂时为空」的结果**永久缓存**，
  导致无论等多久菜单都没有笔记、只有刷新页面才恢复。现在区分「暂时空」（下次打开 `@`
  自动重试）与「确实没有工作区」（照旧缓存）。
- **冲突解决后的 `.conflicts` 残留**：sidecar 清理此前只挂在 AI 推送工具一条路径上——
  经管理器推送或「合并远端并重试」完成解决后，`.dsh-notes/.conflicts/` 目录会残留。
  现在清理移到 `gitPush` 成功出口统一执行（所有推送路径共用，AI 流程不变）。
- **@ 引用不再携带绝对路径**：极小概率（会话工作区列表尚未就绪时选中笔记）引用会在
  消息里序列化出包含本机主目录的绝对路径，并可能随「记入笔记」进入 git 同步的仓库。
  现在这种情况改为不插入引用（重新选择即可）；旧版本已插入的绝对路径 chip 在发送时
  也会改写为「工作区名/.dsh-notes/笔记名」形式——引用从此只含相对路径。
- **记入笔记失败提示的英文泄漏**：写入失败时界面会把 host 侧原始英文错误串（如
  `assistant message not found`）直接拼进本地化文案。现在空回答拒绝改为错误码
  （`empty-answer`）并新增对应文案，所有带错误码的失败按码走本地化，仅无码的传输
  错误才回退原文。
...
- **AI 解决 Git 冲突**：推送拦截与更新冲突的确认弹窗新增「AI 解决」按钮——在冲突工作区
  新建对话，把冲突笔记的本地版/基线/远端版提交给 AI 做语义合并（无法确定的合并点会向
  你提问）；完成后 AI 经 dsh 原生审批面板请求推送，确认后才覆盖远端。详见
  [docs/ai-conflict.md](docs/ai-conflict.md)。
- **笔记搜索**：笔记管理器顶部栏新增搜索框——跨全部工作区的全文搜索（标题 + 正文、
  大小写不敏感；空格分隔多关键词，AND 组合）。结果按工作区分组，展示命中行片段与
  关键词高亮；点命中行在编辑器中打开笔记并定位到该行、关键词处于选中态。见
  [docs/usage.zh.md §2「搜索笔记」](docs/usage.zh.md#搜索笔记)。
- 设置面板的 Git 模式下拉改为 dsh 官方风格自定义菜单（portal 弹层、勾选行、
  外点/Esc 关闭），替代 OS 原生 select 弹出；文本输入框边框与聚焦样式对齐
  harness 设置表单。见 [docs/usage.zh.md §6](docs/usage.zh.md#6-设置面板)。

### Fixed

- 修复网络较慢时「保存笔记卡十多秒」：打开管理器时各工作区的 Git 状态请求原为并发
  （共享仓库下服务端本就串行，并发只是空占连接），加上自动拉取，网慢时占满浏览器同源
  连接池，保存请求被迫排队。现改为逐个串行获取状态，保存不再被 Git 请求阻塞。
- 修复「合并远端并重试」合并冲突后 Git 仓库克隆永久卡死的问题（MERGE_HEAD 未清理导致
  后续所有同步报错）；现在会自动中止残留合并并恢复可用。
...
- 修复 Git 新设备（或新 clone + 空笔记目录）首次同步两条缺陷：① 拉取不生效——「本地
  缺失」被误判为「本地已改」而跳过复制，自动拉取还被「远端无新提交」短路，远端笔记
  拉不下来；② 此时推送会在**无确认**的情况下镜像清空远端全部笔记。现在首次同步前
  基线视为空（同步标记持久化在 clone 的 `.git/` 内，不入库）：首次拉取把远端笔记全部
  带下来，首次推送以「远端有更新」拦截并要求确认。
