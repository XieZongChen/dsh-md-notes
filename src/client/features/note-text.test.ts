...
describe('captureMessageText', () => {
  it('captures the answer and the nearest preceding user question', () => {
    ...
    expect(captureMessageText(nodes, 'm2')).toEqual({ answerText: 'A2', questionText: 'Q2', images: [], producedPaths: [] })
  })
  ...
  it('skips reasoning and keeps only visible text', () => {
    ...
    expect(captureMessageText(nodes, 'm')).toEqual({ answerText: 'visible', questionText: 'Q', images: [], producedPaths: [] })
  })
