...
| `repairSessions` | — | `{ ok, scanned/repaired/events/failed, stillBlocked[] }`（一次性修复旧版 `md-notes` 消息来源 → 官方 `plugin` 最小形态（去额外键，过 v0→v1 精确键校验；幂等可重跑）；首次原件备份不覆盖 + 原子替换，`sessions-repair.ts`） |
