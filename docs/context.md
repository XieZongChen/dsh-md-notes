...
- ✅ **对话内笔记引用可点**（2026-08）：引用约定改为「行内代码包裹相对路径」，客户端经
  `chatFileMentions` 把模型回答里的 `` `相对路径` `` 渲染成可点链接，点击打开管理器跳转（§3.7）
...
- 注入内容带**中英双语引用约定**：回答中引用笔记时用**行内代码包裹笔记的相对路径**
  （`` `.dsh-notes/xxx.md` ``，沿用用户消息里的引用路径）——客户端据此经 `chatFileMentions`
  把该路径渲染成可点链接（点击打开笔记管理器跳转）。这是对模型的尽力引导，非硬性保证。
