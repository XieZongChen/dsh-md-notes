...
- ✅ chip 显示优化：`DshChipCell` 单元格 4em→10em + 标签前置截断（§3.3.1）
...
- ⏳ 10em chip 单元格在浏览器中的实际渲染（字体覆盖是否生效）
...
```markdown
引用笔记「未命名笔记 2026/8/16」：/Users/…/<工作区>/.dsh-notes/xxx.md
```

- `ref` 为笔记的**绝对路径**（含工作区），形如 `<工作区>/.dsh-notes/xxx.md`——**工作区 + 名称**
  天然唯一，跨工作区/同名笔记无歧义；模型据此调用 `read` 读取。
- 标题作为 chip/序列化的可读标签；路径即"授权"——读放行，跨工作区无碍。
- **格式演进**：v1 用 `<note ref="…">标题</note>`（XML 标签，模型可解析，但发送后
  在消息里显示为裸 html 标签，且 `note` 名称过于泛化）；实测后改为**本地化可读行**——
  dsh 的用户消息气泡是纯文本渲染（`projectUserText` 只高亮 `/name`、`@name` 词元），
  任何尖括号标签都无法美化，因此放弃标签语法，用「标题」+ 路径的清晰行式。
...
- **插件修复**（`chip-cell.module.css`）：重新声明同名 `@font-face`（U+FFFC 空白字形
  advance 4000→10000，即 10em；checksum 已修正），chip 与文本框共用同一字体栈，按构造
  对齐不漂移。当前 dsh 核心无其他 chip 来源（skill/subagent 走纯文本引用），放大只影响
  笔记 chip；标签可见宽度约 4em→10em。
- 同时 chip 标签**前置截断**（>18 字符 → 前 18 字符 + …）：即使标题超长，也总是显示
  开头而非中间一截。
