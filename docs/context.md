...
3. **跨工作区**：输入部分工作区名（如 `@dsh-pl`）时，候选列出**模糊匹配的工作区行**
   （`dsh-plugin/`，🗂️ 前缀 + 「工作区」说明）+ 当前工作区过滤后的笔记；**点击工作区行**
   自动补全 `@dsh-plugin/` 并弹出该工作区的笔记列表，继续输入即过滤该工作区的笔记
   （已输入工作区后只显示该工作区）。精确输入 `@dsh-plugin` 也直接切换（**仅 ASCII 工作区名**）。
