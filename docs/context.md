...
- ✅ chip 的 **DOM/尺寸**为 dsh 硬编码（插件不能改结构），但**样式可覆盖**：插件注入的
  `@font-face` 能重定义 `DshChipCell` 的 U+FFFC 宽度，chip 与文本框共用字体栈，按构造对齐
  （见 §3.3.1；候选菜单不可定制渲染，工作区信息走候选 label 文本）
...
- ✅ chip 显示优化：`DshChipCell` 单元格 4em→6em + 标签前置截断（§3.3.1）
- ✅ 序列化格式改为**本地化可读行**（不再输出 `<note>` 标签，见 §3.3）
- 🚧 知识库式自动检索（超出 dsh 原生能力，需自定义，见 §4）

### 待实测（真实会话）

- ⏳ `@` 选笔记（含 `@工作区名/` 跨工作区）→ 发送 → 模型 `read` 读取并在回答中引用
  （**已实测 ✅**：模型正确回答笔记内容；待测跨工作区）
- ⏳ 引用失效路径：删除笔记后发送被阻断，提示「<笔记名> 无法找到，请删除引用」
- ⏳ 6em chip 单元格在浏览器中的实际渲染（字体覆盖是否生效）
...
3. **跨工作区**：输入 `@工作区名` 或 `@工作区名/`（如 `@dsh-plugin`、`@dsh-plugin/`，
   **仅支持 ASCII 工作区名**）扩展到该工作区的笔记（尾斜杠可选，精确匹配工作区名即切换）；
...
- **默认范围**：当前会话工作区的笔记（`api('list', { sessionId })`）——`@` 默认只列当前工作区，
  避免菜单杂乱。
- **跨工作区触发（方式 A）**：输入 `@工作区名` 或 `@工作区名/`（尾斜杠可选）时，candidates
  解析 `query` 的斜杠前段为工作区名，**精确匹配**即返回该工作区的笔记（`@工作区名/笔记` 过滤）；
  未匹配任何工作区名的裸前缀回退为当前工作区过滤。label 为笔记标题，description 可附工作区名。
...
```markdown
引用笔记「未命名笔记 2026/8/16」：dsh-plugin/.dsh-notes/xxx.md
```

- **路径带工作区限定**：`<工作区名>/.dsh-notes/<文件名>`（如 `dsh-plugin/.dsh-notes/3333.md`）——
  首个路径段是工作区名，引用自描述（笔记属于哪个工作区），跨工作区/同名笔记天然无歧义。
- `ref`（chip 身份）即上述路径；`title` 作可读标签。模型据此读取笔记（读放行，见 §3.6；
  若相对解析不到，模型可自行定位绝对路径）。
- **格式演进**：v1 用 `<note ref="…">标题</note>`（XML 标签，模型可解析，但发送后在消息里
  显示为裸 html 标签，且 `note` 名称过于泛化）；v2 改为**本地化可读行**——dsh 的用户消息
  气泡是纯文本渲染（`projectUserText` 只高亮 `/name`、`@name` 词元），任何尖括号标签都
  无法美化，因此放弃标签语法；v3 路径从 `.dsh-notes` 开头（实测模型引用不便）→ 定稿为
  **`<工作区名>/.dsh-notes/…`** 带工作区限定。
...
- dsh 的 chip 单元格宽度 = 文本框里 U+FFFC 占位符的 advance，由 `DshChipCell` 字体
  （只映射 U+FFFC 为空白字形）硬编码为 **4em**；标签在格内居中、overflow 裁剪——长标题
  会被**前后同时截断**（只露中间一截）。
- **插件修复**（`chip-cell.module.css`）：重新声明同名 `@font-face`（U+FFFC 空白字形
  advance 4000→6000，即 **6em**；checksum 已修正），chip 与文本框共用同一字体栈，按构造
  对齐不漂移。当前 dsh 核心无其他 chip 来源（skill/subagent 走纯文本引用），放大只影响
  笔记 chip；**6em** 在标签可读性与短标签的窄外观之间取平衡（10em 实测过宽）。
- 同时 chip 标签**前置截断**（>6 字符 → 前 6 字符 + …）：即使标题超长，也总是显示
  开头而非中间一截（6em 单元可见约 6 字符）。
...
3. **Client source**：`features/ContextSource/` 实现 `InputTriggerSource`
   ...
   - `candidates` 解析 `query`：`@工作区名`（尾斜杠可选）精确匹配 → 该工作区候选，否则当前工作区；
