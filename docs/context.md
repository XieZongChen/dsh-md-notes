...
```markdown
引用笔记「未命名笔记 2026/8/16」：.dsh-notes/xxx.md
```

- **路径从工作区开始**（相对工作区根）：`read` 工具的解析基准是**会话 cwd = 工作区根目录**
  （`exec.agent.session.header.cwd`，tool-fs `session-cwd.ts`），因此**同工作区引用**输出
  `.dsh-notes/xxx.md`，模型按相对路径直接读到正确文件。
- **跨工作区引用**保留**绝对路径**：相对路径没有可靠基准（会话 cwd 是当前工作区，不是目标
  工作区），绝对路径跨工作区读取天然正确（fs 沙箱读放行，见 §3.6）。
- `ref`（chip 身份）即上述序列化路径——**工作区 + 名称**天然唯一；`title` 作可读标签。
- **格式演进**：v1 用 `<note ref="…">标题</note>`（XML 标签，模型可解析，但发送后在消息里
  ...
  无法美化，因此放弃标签语法；v3 路径改为**工作区相对**（同工作区 `.dsh-notes/xxx.md`）。
