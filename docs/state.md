...
| NotesManager 的面板局部 state | L0 | useState | 数据加载/选中/保存反馈/git 组/搜索——按关注点拆在 `features/NotesManager/hooks/`（`useNotesList`/`useNotesEditor`/`useGitSync`，由 `useNotesManager` 编排，后者还持有跨切面的 `gitMsg`/`remoteChanged`/`confirmState`；`useNoteSearch` 与查询框的 query state 由渲染器直接持有，不经编排——搜索态只服务左栏展示、无跨切面读者） |
...
| git 互斥 `KeyedMutex` | L3 | apply 闭包（`src/host/keyed-lock.ts`） | `repo/<repoDir>` 排队串行，GitApi 边界包住 git 顶层调用，防同一 clone 并发竞态 |
