...
| NotesManager 的面板局部 state | L0 | useState | 数据加载/选中/保存反馈/git 组——按关注点拆在 `features/NotesManager/hooks/`（`useNotesList`/`useNotesEditor`/`useGitSync`，由 `useNotesManager` 编排；后者还持有跨切面的 `gitMsg`/`remoteChanged`/`confirmState`） |
