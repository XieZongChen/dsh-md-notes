...
>
> **状态（2026-09-24）：未执行。** 作者读过本评测设计后决定**不再投入**，所以结论是
> 「暂不采用」而不是「伪需求」——`Config.agentTools` 因此**默认 `off`**，本插件对外仍是
> **文档管理器**。本文件与 `scripts/memory-eval.mjs` 原样保留：将来若有人想重开这条线，
> 照着跑即可，判决线不必重新商量（**它是看数之前定的**）。
