...
## 0.9 曾评估过：把笔记当 agent 记忆 —— 已否决（2026-09-24），代码已移除

**结论**：**不做**。理由不是「实现不行」，而是**杠杆点错了**：旧设计里笔记进入模型视野
只有「人用 `@` 贴上去」一条路，天花板就是**人别忘了**；而它的每条承重机制都有一等方对应物
（`@` 路径引用 / 用户级与仓库级 `AGENTS.md` / `session_search`），所以它不太可能提升问答
效果。要让它有意义必须换成「模型自己查、自己写」，但那样一来它就是在造一个 **agent 跨会话
记忆**产品——而这条主张**从未被验证**，作者评估后决定不再投入。**未验证 ≠ 已证伪**，只是
在没有证据前不该把它当卖点，更不该默认开启让用户当实验。

**试过并已删除的东西**（留给未来重启这条线的人，不必从零想）：

- 三个 agent 工具：`note_search`（无关键词时列最近笔记）、`note_read`（文件名/标题读全文，
  跨工作区歧义返回候选**不猜**）、`note_write`（**只追加、绝不覆盖**，`create` 遇已存在文件
  拒绝；与 UI 写入共用 `KeyedLock`）。
- 每会话**一条**「笔记库存在、何时该查」的注入通知（按档位措辞）；「何时该查」的完整策略
  放在**工具描述**里——那是唯一每步可见又不额外花 token 的通道。这是当时的设计要点：工具
  存在 ≠ 模型会用。
- 档位开关 `Config.agentTools: 'off' | 'read' | 'write'`（部署开关，非 volatile）。
- 一套可证伪的评测：`code/README 里推不出事实` 的 fixture + 两臂对照 + **看数之前就定好的
  判决线**（treatment ≥ 5/6 且 delta ≥ +4 → 继续投入；delta ≤ +1 → 判伪需求、停手）。

**实施中踩到、值得记住的两个真问题**（与记忆能力无关，是可复用的教训）：

1. **工具输出 schema 在运行期被校验**：类型不符会让整个工具调用失败（不是降级）。当时
   `note_search` 返回的 `updatedAt` 在 meta 缓存缺失时来自 `stat().mtimeMs`（**浮点**），
   schema 却声明 `integer` → 新建笔记上工具完全不可用。教训：跨边界暴露的数值字段按 schema
   类型显式归一。详见 [coding-standards.md](coding-standards.md) §12 #23。
2. **会话在工作区注册表之外时的写作用域**：原实现会回退到「注册表里只有一个工作区就用它」，
   于是一个 cwd 不在注册表里的会话（无头/自动化/临时目录）可能**往别人真实工作区的笔记里
   追加**。教训：默认目标必须来自「会话自己的上下文」，绝不跨项目兜底。

**顺带确认的一条产品限制**（仍成立，见 README「已知局限」）：插件 `inject` 依赖 `webServer`，
而该服务只由 dsh 的 **web-app** bundle 挂载，`dsh-base` / `dsh-sdk-app` 都没有 → 插件在
**无头 / SDK / ACP** 部署里不会加载。若将来要做任何「agent 侧」能力（含记忆），这是前置：
把 `webServer` 从必需依赖降级为可选服务（取不到就跳过路由与 icon 注册，晚到时用
`ctx.inject(['webServer'], …)` 补注册）。动的是**加载顺序**，必须真机验证 web 侧不回归。
