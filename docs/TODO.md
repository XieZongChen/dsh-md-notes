...
- **若判定 adopt，下一步按价值排序**：
  1. **策展 / provenance**（最该补的一块）：`meta.json` 记录写入者与来源会话
     ...
     「agent 可写」配套的另一半。
  2. **检索排序**：当前是 token AND 子串。笔记上量后需要按标题命中 / 时间 / 互链度数排序
     （向量检索不必需，排序必需）。
  3. **个人跨工作区库**：现在笔记按工作区隔离，用户的跨项目个人知识没有归属。
  4. **让插件能跑在无头部署里**（评测自动化的前置）：`webServer` 由必需依赖降级为可选服务
     ——取不到就跳过路由/icon 注册，晚到时用 `ctx.inject(['webServer'], …)` 补注册（客户端
     半边已有同类晚到激活先例）。动的是**加载顺序**，必须真机验证 web 侧不回归
     （刚修过一次加载期崩溃，风险集中在这里）。解锁后即可用 dsh SDK 跑自动化两臂评测。
- **刻意不做**（评测出结论前）：不再加记忆相关功能，避免价值主张未定前累积负债。
