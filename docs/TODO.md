...
**判定：有影响** —— dsh 移除 `MessageSourceMap` 的泛型 `plugin` source kind（转向 producer-owned kind + Session V4 基线），`@` 引用注入的类型面断裂；`*16` 系列图标改名 `*Medium`，client 10 处 import 断裂。**插件侧适配已完成（2026-09-23，`fix(inject)`/`fix(ui)`/changelog 提交）**；真机冒烟 + 发版入表前，`0.1.7-rc.1`（及途经的 `0.1.5-rc.3`）不入 [compatibility.zh.md](compatibility.zh.md) 对照表。本轮区间 +3305 提交（含 upstream 合并历史），途经 `0.1.5-rc.3` / `0.1.6-alpha.1-2` / `0.1.7-alpha.1-2`；0.1.6 线无 rc 被 0.1.7 收口，按 §0 以最新 rc（`0.1.7-rc.1`，2026-09-23 发布）为目标一次覆盖。插件 `0.13.0` 已定版（无 NEXT_VERSION），因判定有影响，本次不入表。
...
- **需要的动作**：
  ...
  4. ⏳ 历史会话：0.13.0 写入的 `kind: 'plugin'` 事件在 V4 迁移映射为 `plugin:md-notes`（`path` 保留、日志可读），真机确认旧会话可打开、注入行去重正常。
- **阻塞项 / 待验证**：仅剩真机项——旧会话（V3 日志）经 V4 迁移后的打开与注入行去重（类型/机制层面已核实：迁移改写结果与新写入 kind 一致，`path` 保留）。ui-chat 渲染标签已核实（`contextProducer` 默认臂以裸 kind `plugin:md-notes` 标注注入行）。
- **验收标准**：适配后对 `0.1.7-rc.1` typecheck/测试/构建全绿（✅ 已过）+ 真机冒烟（`@` 注入/渲染/跨步去重、NotesManager 图标、旧会话打开）→ 发版 → 重跑 `dsh 兼容性校验` 入表。
