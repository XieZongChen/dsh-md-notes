...
- **4.3 笔记互链 + 反链**（中）：`[[笔记名]]` wiki 链接（或 MarkdownText 的
  `fileMentions` 扩展点）渲染为可点击项，点击在管理器打开目标笔记（复用现有 name 解析
  与 open 路由）；目标不存在时显示失效样式。反链：host 扫描各笔记内容，找出引用当前
  笔记的 `[[…]]` / @ 引用，在编辑器旁展示「谁引用了它」。
  验收：互链点击直达；反链列表准确。
