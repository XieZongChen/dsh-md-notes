...
**fileMentions 笔记跳转已实现**（反引号笔记名 → 点击打开，见 4.3）。
...
- **4.3 笔记互链 + 反链**（中）：✅ **互链基础已实现**——MarkdownText 的 `fileMentions`
  扩展点：预览里反引号包住的笔记名（`` `笔记名` ``，匹配标题或文件名、当前工作区优先）
  渲染为可点击项，点击在编辑器打开目标笔记（复用 `open` 路由）。剩余：
  - `[[笔记名]]` wiki 链接语法（需要自建 mdast 扩展或预处理，评估中）；
  - 目标不存在时的失效样式；
  - 反链：host 扫描各笔记内容，找出引用当前笔记的引用，在编辑器旁展示「谁引用了它」。
  验收：互链点击直达（✅）；反链列表准确。
