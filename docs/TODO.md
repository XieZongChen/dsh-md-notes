...
- **5.5 笔记写入互斥（写锁）+ 全局写入状态**（高）：笔记写操作（记入/保存/删除）全局互斥——
  同一笔记写入期间**任何会话**不可再写（host 权威锁 + client 状态镜像）；笔记入口（含
  tooltip「X 个文件正在写入」）、记入弹窗（不可选中 + loading）、管理器（行 loading +
  隐藏删除 + 操作栏禁用 + 「正在写入文件」提示）三处联动，写入完成自动还原。
  方案见 [write-lock.md](write-lock.md)，状态管理总纲见 [state.md](state.md)。
