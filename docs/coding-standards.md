...
| 23 | ✅ agent 工具输出 schema（`host/note-tools.ts`） | **dsh 在运行期按声明的 schema 校验工具返回值，类型不符会让整个工具调用失败**（不是降级、不是截断）。`note_search` 原样返回 `listNotes` 的 `updatedAt`，而该值在 meta 缓存缺失时来自 `stat().mtimeMs`（**浮点**），schema 却声明 `integer` → 工具直接报 `"value.results[0].updatedAt" must be an integer`，在新建笔记（meta 尚未写入）上完全不可用。typecheck 与单测都发现不了——单测 fixture 恰好用的是整数 | 高 | 已修：在工具边界归一 `Math.trunc(Number(x)) \|\| 0` + 浮点回归测试（真机会话实测暴露）。教训：对外暴露的数值字段要按 schema 类型**显式归一**，别相信上游「看起来是整数」；写 schema 时先问源头的真实类型 |
