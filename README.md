...
- 侧边栏 📓 入口 → 笔记管理界面（列表 + 编辑/预览）
- 每条回答下方 📝 图标 → 把该段对话记入指定笔记
...
配置说明（笔记目录、API 路由）与实现细节见 [docs/design.md](docs/design.md)。
